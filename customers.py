...
######################
@customer_routes.route('/applyjob/<int:job_id>' , methods=['GET','POST'])
...
def download_cv(filename):
    return send_from_directory('mysite/static/uploads/customers/cv', filename)
...
def apply_job_from_company(job_id):
 if "session_ok" in session:
    ...
    # Check if the customer has already applied for this job
    query = customer_jobs.select().where(# noqa: F405
        and_(customer_jobs.c.customer_id == customer_id, customer_jobs.c.job_id == job_id)# noqa: F405
    )
    result = db.session.execute(query).fetchone()# noqa: F405
    ...
    if result:
        ...
        # If the customer has not applied for the job, insert the application
        application = customer_jobs.insert().values(customer_id=customer_id, job_id=job_id , status = status , note = note)# noqa: F405
        db.session.execute(application)# noqa: F405
        db.session.commit()# noqa: F405
        flash('You have successfully applied for the job.')
...
@customer_routes.route('/my-jobapplications')
...
@customer_routes.route('/create_team', methods=['GET'])
def get_create_team():
    if "session_customer" not in session:
        return redirect('/login')
    id = session['user_id']
...
@customer_routes.route('/create_team', methods=['POST'])
def create_team():
...
@customer_routes.route('/add_member_to_team/<int:team_id>', methods=['get'])
...
@customer_routes.route('/add_members_to_team/<int:team_id>', methods=['POST'])
...
@customer_routes.route('/accept_team_invetation')
def accept_team_invetation():
    return "ok"

@customer_routes.route('/profile_team/<int:team_id>')
...
@customer_routes.route('/edit-profile/personal_data', methods=['POST'])
...
@customer_routes.route('/edit-profile/job_data', methods=['POST'])
...
@customer_routes.route('/edit-profile/educational_data', methods=['POST'])
...
@customer_routes.route('/edit-profile', methods=['POST'])
def edit_profile():
