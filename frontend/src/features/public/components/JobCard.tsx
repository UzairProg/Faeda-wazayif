 ...
 *
 * DESIGN PRINCIPLE (Roadmap §2):
 *   Never imply precision or verification that cannot be substantiated.
 *   This card shows only data that comes from the backend.
 *   No fake match percentages, no fabricated ratings, no invented logos.
 *
 * Data fields displayed:
 *   - Job title
 *   - Company name (logo if available, else letter avatar)
 *   - Location / remote indicator
 *   - Work type
 *   - Experience level
 *   - Salary (only if backend discloses it)
 *   - Skills (up to 4, with overflow indicator)
 *   - Posted date
 *   - Apply CTA
 */
import { Link } from "react-router-dom"
import { motion } from "framer-motion"
import { MapPin, Clock, Bookmark, Calendar, Users, ChevronRight } from "lucide-react"
...
function CompanyAvatar({ name, logoUrl }: { name: string; logoUrl: string | null }) {
  ...
  // Letter avatar — first letter of company name
...
export function JobCard({ job, index = 0, className }: JobCardProps) {
  ...
  return (
    <motion.article
      ...
      className={cn(
        "group relative bg-card/40 backdrop-blur-md border border-white/5 rounded-2xl",
    ...
    >
      <div className="p-6">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          {job.isTeamFriendly && (
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-secondary/10 border border-secondary/20 text-xs text-secondary">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-white/5 gap-3">
          <div className="min-w-0">
            {salaryText ? (
              ...
              <p className="text-xs text-muted-foreground/60">الراتب غير محدد</p>
            )}
