      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
          {articles.map((article, index) => (
            ...
            >
              <TiltCard className="h-full rounded-[1.5rem]" tiltMaxAngle={10} shineOpacityMax={0.1}>
              <GlassCard interactive className="flex flex-col h-full group overflow-hidden bg-card/40 backdrop-blur-md border-white/5 text-start shadow-lg transition-all duration-500 ease-out">
                {/* Image Placeholder */}
                <div className={`h-56 w-full relative overflow-hidden`} style={{ transform: "translateZ(30px)" }}>
                  <img src={article.image} alt={article.title} className="w-full h-full object-cover transition-transform duration-700 ease-out" />
                ...
                <div className="p-6 flex flex-col flex-1" style={{ transform: "translateZ(20px)" }}>
                  <h3 className="text-xl font-bold font-heading mb-4 text-white transition-colors line-clamp-2 leading-relaxed">
                    {article.title}
                  </h3>
