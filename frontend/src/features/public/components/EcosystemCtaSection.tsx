/**
 * features/public/components/EcosystemCtaSection.tsx
 *
 * Bottom CTA section for Home page.
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center max-w-2xl mx-auto">
          ...
          <p className="text-sm sm:text-base text-muted-foreground leading-relaxed mb-8">
            {language === "en"
              ? "Whether you are a professional seeking career trajectory growth or an employer recruiting talent and teams, Faeda gives you clarity and control."
          ...
          <div className="flex flex-wrap items-center justify-center gap-6 text-xs text-muted-foreground pt-4 border-t border-white/5">
            <span className="flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5 text-primary" /> {language === "en" ? "Instant Registration" : "تسجيل فوري"}</span>
            <span className="flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5 text-primary" /> {language === "en" ? "Full Data Privacy" : "خصوصية وحماية كاملة"}</span>
            <span className="flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5 text-primary" /> {language === "en" ? "Candidates, Employers & Teams" : "دعم مرشحين وشركات وفرق"}</span>
          </div>
        </div>
      </div>
    </section>
