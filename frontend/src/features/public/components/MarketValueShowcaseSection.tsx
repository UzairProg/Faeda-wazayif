        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          >
            <TiltCard className="rounded-[2rem]" tiltMaxAngle={5} shineOpacityMax={0.15}>
              <GlassCard className="p-6 sm:p-8 bg-card/60 border-white/10 shadow-2xl relative overflow-hidden text-start">
                ...
                <div className="flex items-center justify-between mb-6 pb-4 border-b border-white/10">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-bold font-heading text-white text-base">تقرير القيمة السوقية</h3>
                      <p className="text-xs text-muted-foreground">مهندس برمجيات واجهات قدير • الرياض</p>
                    </div>
                  </div>
                  <span className="px-3 py-1 rounded-full bg-white/5 border border-white/10 text-xs font-semibold text-muted-foreground font-mono">
                    مثال توضيحي
                  </span>
                </div>

                {/* Main Estimated Salary Range Box */}
                <div className="p-6 rounded-2xl bg-gradient-to-r from-primary/20 via-card to-card border border-primary/30 mb-6 relative overflow-hidden">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-3xl sm:text-4xl font-extrabold text-white font-mono tracking-tight">
                        22,000 – 28,000 <span className="text-sm font-sans font-normal text-muted-foreground">ر.س</span>
                      </p>
                    </div>
                    <div className="px-3 py-1.5 rounded-lg bg-primary/20 border border-primary/30 text-primary text-xs font-bold shrink-0">
                      مستوى تنافسي مرتفع
                    </div>
                  </div>
                ...
                <AnimatePresence mode="wait">
                  {activeTab === "overview" && (
                    ...
                    >
                      {demoInputs.map((inp) => (
                        <div key={inp.label} className="p-3.5 rounded-xl bg-white/5 border border-white/5">
                          <span className="text-[11px] text-muted-foreground block mb-1">{inp.label}</span>
                          <p className="text-xs font-bold text-white mb-1">{inp.val}</p>
                          <span className="text-[10px] text-primary font-semibold">{inp.status}</span>
                        </div>
                      ))}
                    </motion.div>
                  )}
                  ...
                  {activeTab === "why" && (
                    ...
                    >
                      <div className="flex flex-wrap gap-2 mb-3">
                        <span className="px-2.5 py-1 rounded-lg bg-primary/10 border border-primary/20 text-primary font-bold text-[11px]">الخبرة (5 سنوات)</span>
                        <span className="px-2.5 py-1 rounded-lg bg-white/5 border border-white/10 text-white font-medium text-[11px]">المهارات (React/TS)</span>
                        <span className="px-2.5 py-1 rounded-lg bg-white/5 border border-white/10 text-white font-medium text-[11px]">التعليم (بكالوريوس)</span>
                        <span className="px-2.5 py-1 rounded-lg bg-white/5 border border-white/10 text-white font-medium text-[11px]">الطلب (الرياض)</span>
                      </div>
                      <div className="p-3 rounded-xl bg-success/10 border border-success/20 text-white flex items-center gap-2">
                        <CheckCircle2 className="w-4 h-4 text-success shrink-0" />
                        <span>امتلاك مهارات React و TypeScript المطلوبة بشدة في مشاريع النواحي التقنية بالرياض.</span>
                      </div>
                      <div className="p-3 rounded-xl bg-warning/10 border border-warning/20 text-white flex items-center gap-2">
                  ...
                  {activeTab === "actions" && (
                    <motion.div
                      ...
                      className="space-y-3 mb-4 text-xs"
                    >
                      <div className="p-3 rounded-xl bg-primary/10 border border-primary/20 text-white flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <Sparkles className="w-4 h-4 text-primary shrink-0" />
                          <span>إضافة شهادة معتمدة في Cloud Architecture (AWS/Azure)</span>
                        </div>
                        <span className="text-primary font-mono font-bold">+2,500 SAR</span>
                      </div>
                      <div className="p-3 rounded-xl bg-primary/10 border border-primary/20 text-white flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <Sparkles className="w-4 h-4 text-primary shrink-0" />
                        ...
                        <span className="text-primary font-mono font-bold">+1,800 SAR</span>
                      </div>
                    </motion.div>
                  )}
                </AnimatePresence>
