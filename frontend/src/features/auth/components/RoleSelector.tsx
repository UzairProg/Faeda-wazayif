...
export function RoleSelector({ selectedRole, onSelectRole }: RoleSelectorProps) {
  return (
    <div className="flex flex-col gap-4 w-full" role="radiogroup" aria-label={auth.register.step1Title}>
        ...
        >
          ...
          <div>
            <h3 className={`font-bold text-sm mb-1 font-heading ${selectedRole === "candidate" ? "text-primary" : "text-white"}`}>
              {auth.roles.candidate.title}
            </h3>
            <p className="text-xs text-muted-foreground leading-relaxed">
              {auth.roles.candidate.description}
            </p>
          </div>
        </div>
        ...
        >
          ...
          <div>
            <h3 className={`font-bold text-sm mb-1 font-heading ${selectedRole === "company" ? "text-primary" : "text-white"}`}>
              {auth.roles.company.title}
            </h3>
            <p className="text-xs text-muted-foreground leading-relaxed">
              {auth.roles.company.description}
            </p>
          </div>
        </div>
      </div>

      {/* Informational Coming Soon Card for University */}
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <div className="flex items-center gap-2 mb-0.5">
              <h4 className="font-bold text-xs text-white/90 font-heading">{auth.roles.university.title}</h4>
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-primary/20 text-primary border border-primary/30">
                ...
                {auth.roles.university.status}
              </span>
            </div>
            <p className="text-[11px] text-muted-foreground/80 leading-normal">
              {auth.roles.university.description}
            </p>
          </div>
        </div>
      </div>
