...
@company_routes.route('/applicants_recommended/<int:job_id>')
...
@company_routes.route('/applicants_approved/<int:job_id>')
...
#################################### rejected####################################
@company_routes.route('/applicants_rejected/<int:job_id>')
...
########################################################################
@company_routes.route('/team_applicant/<int:job_id>/<int:team_id>')
...
########################################################################
@company_routes.route('/update_status/<int:job_id>', methods=['POST'])
...
@company_routes.route('/company_editprofile', methods=['POST'])
...
'''
@company_routes.route('/company_editprofile/public')
...
@company_routes.route('/company_editprofile/public', methods=['POST'])
...
@company_routes.route('/company_editprofile/contact_data')
...
@company_routes.route('/company_editprofile/contact_data', methods=['POST'])
...
@company_routes.route('/see_teams')
def see_teams():
    if "company_id" in session:
            id = session["company_id"]
            company = Company.query.get(id)
...
@company_routes.route('/visit_team_profile/<int:team_id>')
...
@company_routes.route('/company_editprofile/commercial_data')
...
@company_routes.route('/company_editprofile/commercial_data', methods=['POST'])
...
@company_routes.route('/apply/<int:job_id>', methods=['POST'])
def apply(job_id):
    selected_applicants = request.form.getlist('selected_applicants[]')

    if not selected_applicants:
        flash('No applicants selected.')
        return redirect(url_for('some_view'))

    for applicant_id in selected_applicants:
        status = request.form.get(f'status_{applicant_id}')
        note = request.form.get(f'note_{applicant_id}', 'لم يتم إدخال ملاحظة')
        type = "individual"
        team_id = "0"

        try:
            application = customer_jobs.insert().values(
